...
5. Require dependency and icon declaration in each created runtime skill.
   - Tools: none
   - Success: created skill includes `agents/openai.yaml` with `dependencies.tools` entry for MCP `lightrun`, interface icons (`icon_small`, `icon_large`), `brand_color`, and guidance that OAuth must be completed.
   - Failure: dependency/OAuth/icon requirements are absent from created skill metadata or guidance.
...
- `dependencies.tools` entry for MCP `lightrun`
