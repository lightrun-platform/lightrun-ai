...
- A concrete target satisfies the chosen evidence tool's required source fields and selector constraints.
...
- Follow selector requirements and combination semantics exposed by the chosen evidence tool schema.
- Choose the source scope that best answers the measurement question without assuming a universal selector preference order.
...
- If several targets fit, select the strongest candidate(s) using service ownership, environment match, and expected traffic path; ask the user only when viable candidates remain indistinguishable.
...
- Match tools to the requested measurement signal using MCP descriptions and parameter fit.
- Before each action, state what answer or next step the action can change; skip actions that cannot change either.
