...
|---|---|
| `runtime_profile_mcp: unavailable` | Plan the highest-value instrumentation points across all verifiable diff areas; use a small focused set, with 3 as the recommended default cap |
| MCP available, one or more areas `uncovered` | Plan only for Phase 2 `uncovered` areas; use a small focused set, with 3 as the recommended default cap |
...
Identify the top highest-value instrumentation points in the **pre-change** production code for the applicable scope above. Prefer a small focused set; 3 is the recommended default cap unless broader coverage is clearly needed.
...
- Be between code sections (for example between methods or classes)
...
- Use source-root relative file paths (for example `com/example/MyClass.java`) - not full filesystem paths
- Expressions can include variable names, field accesses, method calls, or compound expressions in scope at the line.
- Avoid state-changing expressions (for example calls that mutate objects or global state), and do not use lambdas or anonymous functions as snapshot expressions.
- Objects are captured to 3 nesting levels only - if a deeper field is needed, specify it directly (for example `order.payment.card.last4` instead of `order`)
...
- **How to trigger within the active capture window:** [What traffic or action will hit this line in production. Use the tool's default wait window unless a different window is intentionally chosen.]
