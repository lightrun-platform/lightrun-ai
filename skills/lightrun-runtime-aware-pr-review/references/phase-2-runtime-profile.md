...
Discover whether the **runtime profile** MCP server is available (e.g. `query_hits_state`, `query_variable_values`, `get_snapshot`).
...
- **First**: relax the query within the same `file` — drop `variable` / `value_match` filters, search adjacent lines or sibling methods, and broaden the time window (e.g. drop the `commit` filter to include hits from the last 2 weeks)
...
- **MCP unavailable:** proceed to **Phase 3 — Sampling Plan** (full plan, up to 3 locations) and **Phase 4 — Execute Snapshots**.
...
- **One or more areas `uncovered`:** proceed to **Phase 3 — Sampling Plan** for **uncovered areas only** (up to 3 total), then **Phase 4 — Execute Snapshots** for those locations. Areas already `covered` must not be re-instrumented.
